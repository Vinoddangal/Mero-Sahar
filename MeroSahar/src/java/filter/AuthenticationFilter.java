...
public class AuthenticationFilter implements Filter {
    ...
    private boolean checkLogin(HttpServletRequest req, HttpServletResponse res)
            throws IOException, ServletException, NonexistentEntityException {
        EntityManagerFactory emf = (EntityManagerFactory) req.getServletContext().getAttribute("mero_sahar_emf");
        boolean isUserLoggedIn = false;
        UserTbl user = null;

        UserTblJpaController userjpa = new UserTblJpaController(emf);
        user = userjpa.checkLogin(req.getParameter("email"));

        if (user != null) {
            if (req.getParameter("password").equals(user.getPassword())) //if(BCrypt.checkpw(req.getParameter("password"), user.getPassword()))
            {
                isUserLoggedIn = true;
